...
public final class ScanResultsConfig {
    ...
    public File getLogFile(Context ctx) {
        if (mLogFile == null) {
            mLogFile = new File(ctx.getExternalFilesDir(null), "ScanResults " + Format.LOG_FILENAME.format(getStartTime()) + ".txt");
        }
        return mLogFile;
    }
    ...
    public String[] formatScanResult(int scanCount, int delay, ScanResult sr) {
        ...
        return new String[]{
                ...
                String.format("%11s", Format.newBasic6dec().format(calcTimeDiff(sr))),
